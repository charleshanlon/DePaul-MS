...
public class LinkedIntList {
	...
	private int indexOfH(Node front, int i) {  // TODO
		if (front == null) {
			...
			if (front.item != i) {
				int temp = indexOfH(front.next, i);
	...
	private int sumH(Node front) {  // TODO
		if (front == null) {
			...
			int temp = sumH(front.next);
			return temp + front.item;
		}
	}
	...
	public Node evensH(Node front) {  // TODO
		if (front == null) {
			...
			if (front.item%2 == 0) {
				return new Node(front.item, temp);
			}
			else {
				return temp;
			}
	...
	public Node listAdditionH(Node thisFront, Node otherFront) {  // TODO
		...
		if (thisFront == null || otherFront == null) {
			throw new IllegalArgumentException("this list and the other list have have different lengths");
		}
		Node temp = listAdditionH(thisFront.next, otherFront.next);
		Node add = new Node(thisFront.item + otherFront.item, temp);
		return add;
	}
    ...
    public boolean isSorted() {
        if (first == null)
            return true;
        return isSortedH(first);
    }
    private boolean isSortedH(Node list) {
        if (list == null || list.next == null)
            return true;
        else {
        	return isSortedH(list.next) == (list.item > list.next.item);
        }
	...
	public static void main(String[] args) {
		...
		StdOut.printf("index of 4 = %d ", l.isSorted());
	}
}
